...
private fun LearnMoreCard(onOpenUrl: (String) -> Unit) {
    ...
    TonalCard(onClick = { onOpenUrl(url) }) {
        Row(
            ...
            Column {
                Text(text = stringResource(R.string.home_learn_supersu), style = MaterialTheme.typography.titleSmall)
                Spacer(Modifier.height(4.dp))
                Text(
                    text = stringResource(R.string.home_click_to_learn_supersu),
                    style = MaterialTheme.typography.bodyMedium,
                    color = MaterialTheme.colorScheme.outline
                )
            }
...
private fun previewHomeScreenState(
    ...
    latestVersionInfo = com.android.superuser.ui.util.module.LatestVersionInfo(),
