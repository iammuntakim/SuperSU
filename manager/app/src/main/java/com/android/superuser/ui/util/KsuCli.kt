...
fun flashModule(
    ...
    with(resolver.openInputStream(uri)) {
        ...
        Log.i("SuperSU", "install module $uri result: $result")
...
fun runModuleAction(
    ...
    Log.i("SuperSU", "Module runAction result: $result")
...
fun installBoot(
    ...
    when (lkm) {
        is LkmSelection.LkmUri -> {
            lkmFile = with(resolver.openInputStream(lkm.uri)) {
                val file = File(ksuApp.cacheDir, "supersu-tmp-lkm.ko")
                file.outputStream().use { output ->
                    this?.copyTo(output)
                }
    ...
    val result = flashWithIO("${getKsuDaemonPath()} $cmd", onStdout, onStderr)
    Log.i("SuperSU", "install boot result: ${result.isSuccess}")
