...
pub fn run(package_name: &String, kmi: Option<String>, allow_shell: bool) -> Result<()> {
    ...
    // 1. Check if SuperSU is already loaded
    if ksuinit::has_supersu() {
        info!("SuperSU already loaded, skip loading ko");
    } else {
        ...
        // 3. Get supersu.ko from embedded assets
        let ko_name = format!("{kmi}_supersu.ko");
        let ko_data = assets::get_asset_data(&ko_name)
            .with_context(|| format!("Failed to get {ko_name} from assets"))?;

        // 4. Load supersu.ko from memory with manual relocation
        info!("Loading supersu.ko for KMI {kmi}...");
        let params = if allow_shell {
            cstr!("allow_shell=1")
        } else {
            cstr!("")
        };
        ksuinit::load_module(&ko_data, params).context("Failed to load supersu.ko")?;
        info!("supersu.ko loaded successfully!");
    ...
    info!("Restarting SuperSU Manager {package_name}...");
    ...
    let _ = Command::new("am")
        .args([
            "start",
            "-n",
            &format!("{package_name}/com.android.superuser.ui.MainActivity"),
        ])
        .status();
