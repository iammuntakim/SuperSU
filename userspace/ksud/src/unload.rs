...
/// Find PIDs of processes running in the SuperSU su domain (u:r:ksu:s0).
...
pub fn unload() -> Result<()> {
    info!("unload: starting SuperSU unload sequence");
    ...
    // 4. delete_module("supersu")
    info!("unload: removing supersu module...");
    if let Err(e) = rustix::system::delete_module(c"supersu", 0) {
        warn!("unload: delete_module supersu failed: {e}");
    }
