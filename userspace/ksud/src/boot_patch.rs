...
pub struct BootPatchArgs {
    ...
    /// Do not (re-)install supersu, only modify configs (allow_shell, etc.)
    #[arg(long, default_value = "false")]
    no_install: bool,
...
pub fn patch(args: BootPatchArgs) -> Result<()> {
    let inner = move || {
        ...
        let supersu_ko: Box<dyn AsRef<[u8]>> = if no_install {
            Box::new(Vec::<u8>::new())
        } else if let Some(kmod_path) = kmod {
            Box::new(map_file(&kmod_path)?)
        } else {
            println!("- KMI: {kmi}");
            let name = format!("{kmi}_supersu.ko");
        ...
        if !no_install {
            ...
            println!("- Adding SuperSU LKM");
            let is_supersu_patched = cpio.exists("supersu.ko");

            if !is_supersu_patched && cpio.exists("init") {
                cpio.mv("init", "init.real")?;
            }

            cpio.add("init", CpioEntry::regular(0o755, ksu_init))?;
            cpio.add("supersu.ko", CpioEntry::regular(0o755, supersu_ko))?;

            #[cfg(target_os = "android")]
            if !is_supersu_patched
                && flash
        ...
        if should_write_output {
            ...
            let name = out_name.unwrap_or_else(|| {
                ...
                format!("supersu_patched_{}.img", now.format("%Y%m%d_%H%M%S"))
            });
...
pub fn restore(args: BootRestoreArgs) -> Result<()> {
    ...
    let (mut cpio, vendor_ramdisk_idx) =
        ...
        cpio.exists("supersu.ko"),
        "boot image is not patched by SuperSU"
    );
    ...
    if should_write_output {
        ...
        let name = out_name.unwrap_or_else(|| {
            let now = chrono::Utc::now();
            format!("supersu_restore_{}.img", now.format("%Y%m%d_%H%M%S"))
        });
        let output_image = output_dir.join(name);
...
fn rebuild_without_ksu(
    ...
    println!("- Removing SuperSU from boot image");
    cpio.rm("supersu.ko", false);
