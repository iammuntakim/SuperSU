...
pub fn init() -> Result<()> {
    ...
    if ksuinit::has_supersu() {
        log::info!("SuperSU may be already loaded in kernel, skip!");
    } else {
        log::info!("Loading supersu.ko..");
        if let Err(e) = load_module_from_path("/supersu.ko") {
            log::error!("Cannot load supersu.ko: {:?}", e);
        }
    }
...
fn load_module_from_path(path: &str) -> Result<()> {
    ...
    log::info!("load supersu with params {params:?}");
