...
SuperSU menyediakan mekanisme modul yang mencapai efek memodifikasi direktori sistem dengan tetap menjaga integritas partisi sistem. Mekanisme ini umumnya dikenal sebagai "tanpa sistem".

Mekanisme modul SuperSU hampir sama dengan Magisk. Jika Anda terbiasa dengan pengembangan modul Magisk, mengembangkan modul SuperSU sangat mirip. Anda dapat melewati pengenalan modul di bawah ini dan hanya perlu membaca [difference-with-magisk](difference-with-magisk.md).

::: warning METAMODULE HANYA DIPERLUKAN UNTUK MODIFIKASI FILE SISTEM
SuperSU menggunakan arsitektur [metamodule](metamodule.md) untuk me-mount direktori `system`. **Hanya jika modul Anda perlu memodifikasi file `/system`** (melalui direktori `system`) Anda perlu menginstal metamodule (seperti [meta-overlayfs](https://github.com/tiann/SuperSU/releases)). Fitur modul lainnya seperti skrip, aturan sepolicy, dan system.prop bekerja tanpa metamodule.
:::

## WebUI

Modul SuperSU mendukung tampilan antarmuka dan interaksi dengan pengguna. Lihat [WebUI documentation](module-webui.md) untuk detailnya.

## Konfigurasi Modul

SuperSU menyediakan sistem konfigurasi bawaan yang memungkinkan modul menyimpan pengaturan key-value persisten atau sementara. Untuk detail lebih lanjut, lihat [dokumentasi Konfigurasi Modul](module-config.md).

## Busybox

SuperSU dikirimkan dengan fitur biner BusyBox yang lengkap (termasuk dukungan penuh SELinux). Eksekusi terletak di `/data/adb/ksu/bin/busybox`. BusyBox SuperSU mendukung "Mode Shell Standalone Shell" yang dapat dialihkan waktu proses. Apa yang dimaksud dengan mode mandiri ini adalah bahwa ketika dijalankan di shell `ash` dari BusyBox, setiap perintah akan langsung menggunakan applet di dalam BusyBox, terlepas dari apa yang ditetapkan sebagai `PATH`. Misalnya, perintah seperti `ls`, `rm`, `chmod` **TIDAK** akan menggunakan apa yang ada di `PATH` (dalam kasus Android secara default akan menjadi `/system/bin/ls`, ` /system/bin/rm`, dan `/system/bin/chmod` masing-masing), tetapi akan langsung memanggil applet BusyBox internal. Ini memastikan bahwa skrip selalu berjalan di lingkungan yang dapat diprediksi dan selalu memiliki rangkaian perintah lengkap, apa pun versi Android yang menjalankannya. Untuk memaksa perintah _not_ menggunakan BusyBox, Anda harus memanggil yang dapat dieksekusi dengan path lengkap.

Setiap skrip shell tunggal yang berjalan dalam konteks SuperSU akan dieksekusi di shell `ash` BusyBox dengan mode mandiri diaktifkan. Untuk apa yang relevan dengan pengembang pihak ke-3, ini termasuk semua skrip boot dan skrip instalasi modul.

Bagi yang ingin menggunakan fitur “Standalone Mode” ini di luar SuperSU, ada 2 cara untuk mengaktifkannya:
...
Untuk memastikan semua shell `sh` selanjutnya dijalankan juga dalam mode mandiri, opsi 1 adalah metode yang lebih disukai (dan inilah yang digunakan secara internal oleh SuperSU dan manajer SuperSU) karena variabel lingkungan diwariskan ke proses anak.

::: perbedaan tip dengan Magisk

BusyBox SuperSU sekarang menggunakan file biner yang dikompilasi langsung dari proyek Magisk. **Berkat Magisk!** Oleh karena itu, Anda tidak perlu khawatir tentang masalah kompatibilitas antara skrip BusyBox di Magisk dan SuperSU karena keduanya persis sama!
:::

## SuperSU module

Modul SuperSU adalah folder yang ditempatkan di `/data/adb/modules` dengan struktur di bawah ini:
...
::: perbedaan tip dengan Magisk
SuperSU tidak memiliki dukungan bawaan untuk Zygisk, jadi tidak ada konten terkait Zygisk dalam modul. Namun, Anda dapat menggunakan [ZygiskNext](https://github.com/Dr-TSNG/ZygiskNext) untuk mendukung modul Zygisk. Dalam hal ini, konten modul Zygisk identik dengan yang didukung oleh Magisk.
:::

### module.prop

module.prop adalah file konfigurasi untuk sebuah modul. Di SuperSU, jika modul tidak berisi file ini, maka tidak akan dikenali sebagai modul. Format file ini adalah sebagai berikut:
...
::: perbedaan tip dengan Magisk
Anda dapat menggunakan variabel lingkungan KSU untuk menentukan apakah skrip berjalan di SuperSU atau Magisk. Jika berjalan di SuperSU, nilai ini akan disetel ke true.
:::
...
Anda juga dapat mendeklarasikan variabel bernama `REMOVE` yang berisi daftar direktori di `customize.sh` untuk menjalankan operasi penghapusan, dan SuperSU akan secara otomatis mengeksekusi `mknod <TARGET> c 0 0` di direktori modul yang sesuai. Misalnya:
...
Anda dapat mendeklarasikan variabel bernama `REPLACE` di file `customize.sh` Anda, yang menyertakan daftar direktori yang akan diganti, dan SuperSU akan secara otomatis melakukan operasi yang sesuai di direktori modul Anda. Misalnya:
...
Penginstal modul SuperSU adalah modul SuperSU yang dikemas dalam file zip yang dapat di-flash di aplikasi pengelola SuperSU. Pemasang modul SuperSU yang paling sederhana hanyalah modul SuperSU yang dikemas sebagai file zip.
...
:::peringatan
Modul SuperSU TIDAK didukung untuk diinstal dalam pemulihan kustom!!
:::
...
Skrip `customize.sh` berjalan di shell BusyBox `ash` SuperSU dengan "Mode Mandiri" diaktifkan. Variabel dan fungsi berikut tersedia:

#### Variable

- `KSU` (bool): variabel untuk menandai bahwa skrip berjalan di lingkungan SuperSU, dan nilai variabel ini akan selalu benar. Anda dapat menggunakannya untuk membedakan antara SuperSU dan Magisk.
- `KSU_VER` (string): string versi dari SuperSU yang diinstal saat ini (mis. `v0.4.0`)
- `KSU_VER_CODE` (int): kode versi SuperSU yang terpasang saat ini di ruang pengguna (mis. `10672`)
- `KSU_KERNEL_VER_CODE` (int): kode versi SuperSU yang terpasang saat ini di ruang kernel (mis. `10672`)
- `BOOTMODE` (bool): selalu `true` di SuperSU
...
::: peringatan
Di SuperSU, MAGISK_VER_CODE selalu 25200 dan MAGISK_VER selalu v25.2. Tolong jangan gunakan kedua variabel ini untuk menentukan apakah itu berjalan di SuperSU atau tidak.
:::
...
Di SuperSU, skrip dibagi menjadi dua jenis berdasarkan mode operasinya: mode post-fs-data dan mode layanan late_start:
...
Di SuperSU, skrip startup dibagi menjadi dua jenis berdasarkan lokasi penyimpanannya: skrip umum dan skrip modul:
...
Semua skrip boot akan berjalan di shell BusyBox `ash` SuperSU dengan "Mode Mandiri" diaktifkan.

## Mode late-load {#late-load-mode}

Selain alur boot standar yang dijelaskan di atas, SuperSU mendukung **mode late-load** untuk skenario LKM (Loadable Kernel Module). Dalam mode ini, modul kernel SuperSU dimuat **setelah sistem sepenuhnya boot**, bukan selama proses init.

### Kapan late-load terjadi?

Late-load dipicu dengan menjalankan perintah `ksud late-load`. Perintah ini:

1. Mendeteksi versi KMI saat ini dan memuat `supersu.ko` yang sesuai dari aset tertanam.
...
```txt
ksud late-load:
  1. Muat supersu.ko (jika belum dimuat)
