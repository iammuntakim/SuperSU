...
SuperSU menyediakan sistem konfigurasi bawaan yang memungkinkan modul menyimpan pengaturan key-value persisten atau sementara. Konfigurasi disimpan dalam format biner di `/data/adb/ksu/module_configs/<module_id>/` dengan karakteristik berikut:
...
Modul dapat mendeklarasikan fitur SuperSU mana yang mereka kelola menggunakan pola konfigurasi `manage.<feature>`. Fitur yang didukung sesuai dengan enum internal `FeatureId` SuperSU:
...
- Pengelola SuperSU mendeteksi modul mana yang mengelola fitur SuperSU mana
- Pencegahan konflik ketika beberapa modul mencoba mengelola fitur yang sama
- Koordinasi yang lebih baik antara modul dan fungsionalitas inti SuperSU

::: warning HANYA FITUR YANG DIDUKUNG
Gunakan hanya nama fitur yang telah ditentukan sebelumnya yang tercantum di atas (`su_compat`, `kernel_umount`). Ini sesuai dengan fitur internal SuperSU yang sebenarnya. Menggunakan nama fitur lain tidak akan menyebabkan error, tetapi tidak memiliki tujuan fungsional.
:::
