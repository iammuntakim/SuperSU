...
Since SuperSU v1.0, we have dropped official support for non-GKI devices.
:::

SuperSU can be integrated into non-GKI kernels and was backported to 4.14 and earlier versions.

Due to the fragmentation of non-GKI kernels, we don't have a universal way to build them; therefore, we cannot provide a non-GKI boot.img. However, you can build the kernel with SuperSU integrated on your own.

First, you should be able to build a bootable kernel from kernel source code. If the kernel isn't open source, then it is difficult to run SuperSU for your device.

If you're able to build a bootable kernel, there are two ways to integrate SuperSU into the kernel source code:
...
SuperSU uses kprobe for its kernel hooks. If kprobe runs reliably on your kernel, we recommend integrating SuperSU this way.

First, add SuperSU to your kernel source tree:
...
::: info
[SuperSU 1.0 and later versions no longer support non-GKI kernels](https://github.com/tiann/SuperSU/issues/1705). The last supported version is `v0.9.5`, so make sure to use the correct version.
:::
...
Now, when you re-build your kernel, SuperSU should work correctly.
...
However, if you encounter a bootloop after integrating SuperSU, this may indicate that the **kprobe is broken in your kernel**, which means that you should fix the kprobe bug or use another way.

::: tip HOW TO CHECK IF KPROBE IS BROKEN？
Comment out `ksu_sucompat_init()` and `ksu_ksud_init()` in `SuperSU/kernel/ksu.c`. If the device boots normally, kprobe may be broken.
:::
...
Next, add SuperSU calls to the kernel source. Below are some patches for reference:
...
To enable SuperSU's built-in Safe Mode, you should modify the `input_handle_event` function in `drivers/input/input.c`:
