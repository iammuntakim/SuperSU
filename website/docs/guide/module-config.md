...
SuperSU provides a built-in configuration system that allows modules to store persistent or temporary key-value settings. Configurations are stored in a binary format at `/data/adb/ksu/module_configs/<module_id>/` with the following characteristics:
...
Modules can declare which SuperSU features they manage using the `manage.<feature>` configuration pattern. The supported features correspond to SuperSU's internal `FeatureId` enum:
...
- SuperSU manager to detect which modules manage which SuperSU features
- Prevention of conflicts when multiple modules try to manage the same feature
- Better coordination between modules and core SuperSU functionality

::: warning SUPPORTED FEATURES ONLY
Only use the predefined feature names listed above (`su_compat`, `kernel_umount`). These correspond to actual SuperSU internal features. Using other feature names will not cause errors but serves no functional purpose.
:::
