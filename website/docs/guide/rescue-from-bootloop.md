...
In SuperSU, the following situations may cause boot brick when flashing the boot partition:
...
If you have flashed a module that has been proven to be safe but causes your device to fail to boot, then this situation is easily recoverable in SuperSU without any worries. SuperSU has built-in mechanisms to rescue your device, including the following:
...
1. The built-in Safe Mode of some systems: Some systems have a built-in Safe Mode that can be accessed by long-pressing the Volume down button. In other systems (such as HyperOS), Safe Mode can be activated from the Recovery. When entering the system's Safe Mode, SuperSU will also enter this mode and automatically disable the modules.
2. The built-in Safe Mode of SuperSU: In this case, the method is to **press the Volume down key continuously more than three times** after the first boot screen.

After entering Safe Mode, all modules on the Module page in the SuperSU manager will be disabled. However, you can still perform the "uninstall" operation to remove any modules that may be causing issues.
