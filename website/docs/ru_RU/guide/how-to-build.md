...
Начиная с SuperSU v3.0, мы отказались от официальной поддержки режима образов GKI для более быстрой итерации и скорости сборки. Рекомендуется использовать `Ylarod/ddk` для сборки LKM.
:::
...
::: warning
Эта страница предназначена для устройств GKI, если вы используете старое ядро, пожалуйста, обратитесь к [Как интегрировать SuperSU для не GKI ядер?](how-to-integrate-for-non-gki).
:::
...
## Сборка ядра с помощью SuperSU

Если вы успешно собрали ядро, то собрать SuperSU очень просто, выберите любой запуск в корневом каталоге исходного кода ядра:
...
А затем пересоберите ядро и получите образ ядра с SuperSU!
