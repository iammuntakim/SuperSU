...
Metamódulos são um recurso revolucionário no SuperSU que transfere recursos críticos do sistema de módulos do daemon principal para módulos plugáveis. Essa mudança arquitetônica mantém a estabilidade e segurança do SuperSU enquanto libera um maior potencial de inovação para o ecossistema de módulos.

## O que é um Metamódulo?

Um metamódulo é um tipo especial de módulo SuperSU que fornece funcionalidade de infraestrutura central para o sistema de módulos. Ao contrário dos módulos regulares que modificam arquivos do sistema, os metamódulos controlam *como* os módulos regulares são instalados e montados.

Metamódulos são um mecanismo de extensão baseado em plugins que permite a personalização completa da infraestrutura de gerenciamento de módulos do SuperSU. Ao delegar a lógica de montagem e instalação aos metamódulos, o SuperSU evita se tornar um ponto de detecção frágil enquanto permite diversas estratégias de implementação.
...
Soluções root tradicionais incorporam a lógica de montagem em seu núcleo, tornando-as mais fáceis de detectar e mais difíceis de evoluir. A arquitetura de metamódulos do SuperSU resolve esses problemas através da separação de preocupações.

**Vantagens estratégicas:**

- **Superfície de detecção reduzida**: O próprio SuperSU não realiza montagens, reduzindo vetores de detecção
- **Estabilidade**: O daemon central permanece estável enquanto as implementações de montagem podem evoluir
- **Inovação**: A comunidade pode desenvolver estratégias alternativas de montagem sem bifurcar o SuperSU
...
- **Extensibilidade**: Adicione recursos como suporte a módulos do kernel sem modificar o núcleo do SuperSU
- **Modularidade**: Atualize implementações independentemente das versões do SuperSU
- **Personalização**: Crie soluções especializadas para dispositivos ou casos de uso específicos

::: warning IMPORTANTE
Sem um metamódulo instalado, os módulos **NÃO** serão montados. Instalações novas do SuperSU requerem a instalação de um metamódulo (como `meta-overlayfs`) para que os módulos funcionem.
:::
...
2. Abra o aplicativo SuperSU Manager
...
Você pode verificar qual metamódulo está atualmente ativo na página de Módulos do aplicativo SuperSU Manager. O metamódulo ativo será exibido na sua lista de módulos com sua designação especial.
...
Apenas um metamódulo pode ser instalado por vez. Se você tentar instalar um segundo metamódulo, o SuperSU impedirá a instalação para evitar conflitos.
...
Se você está desenvolvendo módulos SuperSU regulares, não precisa se preocupar muito com metamódulos. Seus módulos funcionarão desde que os usuários tenham um metamódulo compatível (como `meta-overlayfs`) instalado.
...
::: tip
Se você está familiarizado com o desenvolvimento de módulos Magisk, seus módulos funcionarão da mesma forma no SuperSU quando o metamódulo estiver instalado, pois ele fornece montagem compatível com Magisk.
:::

## Para Desenvolvedores de Metamódulos

Criar um metamódulo permite que você personalize como o SuperSU lida com instalação de módulos, montagem e desinstalação.
...
- Todas as variáveis de ambiente padrão do SuperSU
...
Ao realizar operações de montagem, você **DEVE** definir o nome da origem/dispositivo como `"KSU"`. Isso identifica as montagens como pertencentes ao SuperSU.
...
Isso é essencial para o SuperSU identificar e gerenciar adequadamente suas montagens.
:::
...
Quando um metamódulo é instalado, o SuperSU cria um symlink:
...
1. **Teste a instalação** em uma configuração limpa do SuperSU
...
- [Diferença com Magisk](difference-with-magisk.md) - Comparando SuperSU e Magisk
- [Como Compilar](how-to-build.md) - Compilando SuperSU a partir do código-fonte
