# Library for SuperSU's module WebUI
...
```javascript
import { fullScreen } from 'supersu';
fullScreen(true);
```
...
- To get insets value and enable this automatically, you can
  - add `@import "https://mui.supersu.org/internal/insets.css";` in css OR
  - add `<link rel="stylesheet" type="text/css" href="/internal/insets.css" />` in html.

```javascript
import { enableEdgeToEdge } from 'supersu';
...
```javascript
import { moduleInfo } from 'supersu';
...
```javascript
import { listPackages } from 'supersu';
// list user packages
...
```javascript
import { getPackagesInfo } from 'supersu';
