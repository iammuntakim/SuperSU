...
- 基于内核的 `su` 和权限管理。
- 基于 [metamodules](https://supersu.org/zh_CN/guide/metamodule.html) 的模块系统：可插拔的模块架构。
- [App Profile](https://supersu.org/zh_CN/guide/app-profile.html): 把 Root 权限关进笼子里。

## 兼容状态

SuperSU 官方支持 GKI 2.0 的设备（内核版本5.10以上）；旧内核也是兼容的（最低4.14+），不过需要自己编译内核。

WSA, ChromeOS 和运行在容器上的 Android 也可以与 SuperSU 一起工作。

目前支持 `arm64-v8a` 和 `x86_64` 架构。

> [!CAUTION]
> 最近的内核版本引入了一项破坏性更改，导致 SuperSU 在 `x86_64` 上运行失败，甚至可能引发内核恐慌 (kernel panic)！请查看网站获取更多信息！

## 使用方法

- [安装教程](https://supersu.org/zh_CN/guide/installation.html)
- [如何构建？](https://supersu.org/zh_CN/guide/how-to-build.html)
- [官方网站](https://supersu.org/zh_CN/)

## 参与翻译

要将 SuperSU 翻译成您的语言，或完善现有的翻译，请使用 [Weblate](https://hosted.weblate.org/engage/supersu/)。现已不再接受有关管理器翻译的PR，因为这会与Weblate冲突。
...
有关报告 SuperSU 安全漏洞的信息，请参阅 [SECURITY.md](/SECURITY.md)。
...
- [kernel-assisted-superuser](https://git.zx2c4.com/kernel-assisted-superuser/about/)：SuperSU 的灵感。
